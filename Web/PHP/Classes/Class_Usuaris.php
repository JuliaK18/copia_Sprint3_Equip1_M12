...
class Usuari {
    ...
    public function update_to_verify_user(){
        include 'connect.php';

        
        //Establim la consula a la base de dades
        $sql = "UPDATE `Usuari` SET `Verificat` = '1' WHERE `Usuari`.`Id` = $this->id";
    ...
    public function update_to_not_verify_user(){
        include 'connect.php';
        //Establim la consula a la base de dades
        $sql = "UPDATE `Usuari` SET `Verificat` = '0' WHERE `Usuari`.`Id` =  $this->id";
        //Executem la consulta
        $query_run = $conn->query($sql);
        mysqli_close($conn);  
    }
    ...
    public static function get_list_users(){
        ...
        $sql = 'SELECT Id, NomUsuari, Verificat, Acceptat FROM Usuari';
        
        $query_run = mysqli_query($conn, $sql);
        mysqli_close($conn);
        return $query_run->fetch_all(MYSQLI_ASSOC);  
    }
